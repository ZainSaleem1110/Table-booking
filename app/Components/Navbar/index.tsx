    ...
    <>
      <div className='bg-[#003b95] flex flex-col justify-center items-center pt-3 py-16 w-full'>
        <div className='h-[70px] xl:w-[60%] lg:w-[80%] w-[90%] text-white flex items-center justify-between'>
          ...
          <div className="navIcon text-[42px] cursor-pointer" onClick={() => { setOpen(true) }}>
            <IoReorderThree />
          </div>
        </div>
        ...
        <div className='flex justify-start pt-14 xl:w-[60%] lg:w-[80%] w-[90%]'>
          <div className='w-full relative '>
            <h2 className='text-white md:text-[42px] sm:text-[32px] text-[18px] font-bold sm:text-start text-center'>Find your table for any cuisines</h2>
            <p className='text-white md:text-[24px] sm:text-[20px] text-[14px] sm:text-start text-center sm:mt-0 mt-2'>Make a free reservation and search best restaurants for good meals.</p>
          </div>
        </div>
      </div>
      <div className={`fixed top-0 h-[100vh] sm:w-[350px] w-[300px] border bg-white px-7 py-5 z-10 ${open === false ? 'sm:right-[-360px] right-[-300px]' : 'right-0'} duration-1000 transition-all`}>
